...
class biasPredictor():
    ...
    def __init__(self, method):
        ...
        elif method == Model.TFIDF:
            self.model: RandomForestClassifier = Config.TFIDF_MODEL
            self.vectorizer: TfidfVectorizer = Config.TFIDF_VECTORIZER
    ...
    def predict(self, explain=False, **kwargs) -> Union[Dict[str, float], Tuple[Dict[str, float], Any]]:
        ...
        if self.method == Model.BERT:
            ...
            cl_values = self.model.predict(prediction[1].cpu().detach().numpy())[0]
        ...
        elif self.method == Model.TFIDF:
            text_vector = self.vectorizer.transform([text])
            # cl = self.model.predict(text_vector)[0]
            cl = self.model.predict_proba(text_vector)
            cl_values: Dict[str, float] = {v: cl[0][k] for k, v in self.LABELS_DICT.items()}
            if explain:
                explanation = list(self.explain_rf_(text_vector))

        if explain:
            return cl_values, explanation
        else:
            return cl_values
